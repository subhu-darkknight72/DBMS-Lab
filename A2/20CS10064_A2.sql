...
CREATE TABLE Affiliated_With(
    ...
    CONSTRAINT pk PRIMARY KEY(Physician, Department),
...
CREATE TABLE Prescribes(
    ...
    Appointment INT,
    Dose varchar(50),
    CONSTRAINT pk PRIMARY KEY(Physician, Patient, Medication, Date),
    FOREIGN KEY(Physician) REFERENCES Physician(EmployeeID),
    FOREIGN KEY(Patient) REFERENCES Patient(SSN),
...
CREATE TABLE `Block`(
    `Floor` INT NOT NULL,
    Code INT NOT NULL,
    CONSTRAINT pk PRIMARY KEY(`Floor`, Code)
);
...
CREATE TABLE Room(
    ...
    -- FOREIGN KEY(BlockFloor) REFERENCES `Block`(`Floor`),
...
CREATE TABLE On_Call(
    ...
    `Start` TIMESTAMP,
    `End` TIMESTAMP,
    CONSTRAINT pk PRIMARY KEY(Nurse, BlockFloor, BlockCode, `Start`, `End`)
);
...
CREATE TABLE Undergoes(
    ...
    Physician INT NOT NULL,
    AssistingNurse INT,
    CONSTRAINT pk PRIMARY KEY(Patient, `Procedure`, Stay, `Date`),
    FOREIGN KEY(Patient) REFERENCES Patient(SSN),
...
CREATE TABLE Trained_In(
    ...
    CONSTRAINT pk PRIMARY KEY(Physician, Treatment),
    FOREIGN KEY(Physician) REFERENCES Physician(EmployeeID),
...
FROM Nurse
WHERE EmployeeID IN (
    SELECT Undergoes.AssistingNurse
    FROM Undergoes JOIN `Procedure`
        ON Undergoes.`Procedure` = `Procedure`.Code
    WHERE `Procedure`.`Name`="bypass surgery"
);
...
WHERE (Nurse.EmployeeID, Physician.EmployeeID) IN (
    SELECT Undergoes.AssistingNurse, Undergoes.Physician
    FROM Undergoes JOIN `Procedure`
        ON Undergoes.`Procedure` = `Procedure`.Code
    WHERE `Procedure`.`Name`="bypass surgery"
);
...
-- Q8
SELECT `Name` as 'Physician Name'
FROM Physician
WHERE EmployeeID IN (
    SELECT Undergoes.Physician
    FROM Undergoes
    WHERE (Undergoes.Physician, Undergoes.`Procedure`) NOT IN(
...
WHERE EmployeeID IN (
    SELECT Undergoes.Physician
    FROM Undergoes
    WHERE (Undergoes.Physician, Undergoes.`Procedure`) IN(
        SELECT Physician, Treatment
        FROM Trained_In
        WHERE TIMESTAMPDIFF(SECOND, Trained_In.CertificationExpires, Undergoes.Date)>0
    )
);
...
SELECT 
    ...
    `Procedure`.`Name` AS Treatment, 
    Undergoes.`Date`, 
    Patient.Name AS 'Patient'
FROM 
    Physician, `Procedure`, Undergoes, Patient
WHERE (Undergoes.Physician, Undergoes.`Procedure`) IN(
        SELECT Physician, Treatment
        FROM Trained_In
        WHERE TIMESTAMPDIFF(SECOND, Trained_In.CertificationExpires, Undergoes.Date)>0
...
-- Q11
-- WITH cte0(Patient, Physician) AS (
--     SELECT Patient, Physician
--     FROM Prescribes
--     WHERE Medication IS NOT NULL
-- ),
-- WITH cte1(Patient) AS (
--     SELECT Patient
--     FROM Undergoes JOIN `Procedure`
--         ON Undergoes.`Procedure`=`Procedure`.Code
--     WHERE `Procedure`.Cost>5000
-- )
-- WITH cte2(Patient) AS (
    
)
-- WITH cte3(EmployeeID) AS (
--     SELECT EmployeeID
--     FROM Physician
...
FROM Patient
JOIN Physician
WHERE (Patient.SSN, Physician.EmployeeID) IN (
    SELECT Patient, Physician
        FROM Prescribes
        WHERE Medication IS NOT NULL
        )
...
-- Q12
SELECT Medication.`Name`, Medication.Brand, COUNT(Prescribes.Patient) AS Qty
