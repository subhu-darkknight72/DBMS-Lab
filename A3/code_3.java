    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (x == 0)
                break;
            if (x > 13 | x < 0) {
                System.out.println("Invalid Command No.");
                continue;
            }
            String cmd;
            if (x == 13) {
                ...
                cmd = "SELECT `Name` FROM Physician WHERE EmployeeID IN (SELECT Physician FROM Trained_In WHERE Treatment IN (    SELECT Code    FROM `Procedure`    WHERE `Name`='"
                        + proc + "'));";
            ...
            try {
                ...
                for (int i = 1; i <= columnCount; i++) {
                    ...
                    int l = rsmd.getColumnName(i).length();
                    for (int j = 0; j < (20 - l - 1); j++)
                        System.out.print(" ");
                    System.out.print("|");
                }
                System.out.println();
