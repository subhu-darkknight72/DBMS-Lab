...
while (ch=="1") :
  ...
  if choice=="1" :
    ...
    schema.execute("SELECT Name FROM Nurse WHERE EmployeeID in (SELECT On_Call.Nurse FROM On_Call INNER JOIN Room ON Room.BlockCode=On_Call.BlockCode AND Room.BlockFloor=On_Call.BlockFloor WHERE Room.Number='123')")
    output=schema.fetchall()
    for x in output:
      print(x)
  elif choice=="4" :
